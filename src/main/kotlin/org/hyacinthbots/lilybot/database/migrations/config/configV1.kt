...
suspend fun configV1(configDb: MongoDatabase) {
	with(configDb.getCollection<LoggingConfigData>(LoggingConfigCollection.name)) {
		updateMany(
	...
	configDb.getCollection<LoggingConfigData>(LoggingConfigCollection.name).updateMany(
		Filters.empty(),
