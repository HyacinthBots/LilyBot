        ...
        event<ChannelCreateEvent> {
            check {
                failIf {
                    ...
                    event.channel.type != ChannelType.PublicGuildThread ||
                        event.channel.type != ChannelType.PrivateThread
                }
            }
            action {
...
// 					}
                ...
                getLoggingChannelWithPerms(ConfigOptions.UTILITY_LOG, guild)?.createEmbed {
                    ...
                    footer {
                        text = "Originally created by ${
                            event.scheduledEvent.creatorId?.let { guild.getMemberOrNull(it) }?.username
                        }"
                        icon =
                            event.scheduledEvent.creatorId?.let {
                                guild.getMemberOrNull(it)
                            }?.avatar?.cdnUrl?.toUrl()
                    }
                }
            }
        ...
        event<InviteCreateEvent> {
            check { anyGuild() }
            action {
                val config = event.guildId?.let { UtilityConfigCollection().getConfig(it) }
                // Do not log if invite updates are disabled
                if (config?.logInviteUpdates == false) return@action
        ...
        event<RoleUpdateEvent> {
            action {
                ...
                if (event.old?.name == event.role.name && event.old?.hoisted == event.role.hoisted &&
                    event.old?.mentionable == event.role.mentionable &&
                    event.old?.getPosition() == event.role.getPosition() && event.old?.icon == event.role.icon &&
                    event.old?.unicodeEmoji == event.role.unicodeEmoji &&
        ...
        event<ThreadChannelCreateEvent> {
            ...
            action {
                val config = UtilityConfigCollection().getConfig(event.channel.guild.id)
                // Do not log if the channel logging option is false
                if (config?.logChannelUpdates == false) {
                    return@action
                }
        ...
        event<ThreadChannelDeleteEvent> {
            ...
            action {
                // Do not log if the channel logging option is false
                if (UtilityConfigCollection().getConfig(event.channel.guild.id)?.logChannelUpdates == false) {
                    return@action
                }
                getLoggingChannelWithPerms(ConfigOptions.UTILITY_LOG, event.channel.guild)?.createEmbed {
                    title = translations.deleteTitle.translate(writeChannelType(event.channel.type))
    ...
    private suspend fun EmbedBuilder.guildEventEmbed(event: GuildScheduledEventEvent, guild: GuildBehavior) {
        ...
        field {
            ...
            value = if (event.scheduledEvent.channelId != null) {
                guild.getChannelOrNull(event.scheduledEvent.channelId!!)?.mention
                    ?: Translations.Basic.UnableTo.channel.translate()
            } else {
