...
suspend inline fun CheckContext<*>.configPresent() {
	...
	if (DbGetters.getModerationConfig(guildFor(event)!!.id) == null) {
...
suspend inline fun EphemeralSlashCommandContext<*>.isBotOrModerator(user: User, commandName: String): String? {
	val moderatorRoleId = DbGetters.getModerationConfig(guild!!.id)?.team
	if (DbGetters.getModerationConfig(guild!!.id)!!.enabled) {
		respond {
